...
dat$interaccion <- interaction(dat$Sp.,dat$Size,sep = ":")
...
# Final analysis previous to the pairwise t tests, the R2 of the full model

R2.rob3 <- cor(modelo.lmrob3$model[,1],modelo.lmrob3$fitted.values)^2
print(c("R2 =", round(R2.rob3,3)), quote="FALSE")
...
dat$perc_col <- dat$S_shoots/dat$S_substratum
...
# Wilcoxon tests for the table S.3 ----------------------------------------------------------
...
options(contrasts=c(factor="contr.sum", ordered="contr.poly")) ## We have factors in the models, so we change the contrast matrix as it do not corresponds with the theory in stadistics, by deafult R applies a different one
dat2$perc_col <- dat2$S_shoots/dat2$S_substratum
...
dat4names <- c("Shoot percentage", "Apparent viability","Area (mm\U00B2)","Length (mm)","Circularity","Wet vs dry area","Wet vs dry length",
...
dat5 <- dat4[,c("Shoot percentage", "Apparent viability","Area (mm\U00B2)","Length (mm)","Circularity","Wet vs dry area","Wet vs dry length")]
